...
import { apiService, Finding, AnalysisStatus } from '@/lib';
  ...
  const uploadMutation = useMutation({
    mutationFn: async (file: File) => {
      ...
      try {
        ...
        updateProcessStep('upload-start', {
          status: 'completed',
          message: `Upload completed! Document ID: ${result.file_hash.substring(0, 8)}...`,
          icon: <CheckCircle2 className="w-4 h-4" />
        });
        ...
        addProcessStep({
          ...
          status: 'completed',
          message: `Extracted ${result.total_pages} pages and ${result.total_chunks} text chunks.`,
    ...
    onSuccess: (data) => {
      ...
      setAnalysisState(prev => ({
        ...prev,
        documentId: data.file_hash,
        filename: data.file_hash,
        isUploading: false,
        isAnalyzing: true,
      }));
